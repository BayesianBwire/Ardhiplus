...
def check_csrf():
    # Expect header 'X-CSRF-Token' or cookie 'csrf_token' to match session token
    token = request.headers.get("X-CSRF-Token") or request.headers.get("X-XSRF-TOKEN") or request.cookies.get("csrf_token")
...
def api_post_listing():
    ...
    # If submitted as multipart/form-data, allow coords string or coords_lat / coords_lng fallback
    if not isinstance(coords, dict):
        coords_str = coords if isinstance(coords, str) else None
        if coords_str:
            try:
                parts = [part.strip() for part in coords_str.split(",")]
                lat = float(parts[0]) if len(parts) >= 2 else None
                lng = float(parts[1]) if len(parts) >= 2 else None
